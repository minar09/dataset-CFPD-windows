...
def make_color_tsv(mat_file='./fashon_parsing_data.mat'):
    f = h5py.File(mat_file, 'r')
    all_ctgs = get_all_colors(f)

    # save
    sr = pd.Series(all_ctgs)
    sr.name = 'color'
    sr.index.name = 'color_id'
    sr.to_csv('./label/colors.tsv',
              sep='\t', header=True)
